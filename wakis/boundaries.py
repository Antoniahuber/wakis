...
class BCsMixin:
    def _apply_bc_to_C(self):
        ...
        if self.use_mpi and self.grid.use_mpi:
            if (
                ...
                or self.bc_high[2].lower() == "periodic"
            ):
                raise NotImplementedError(
                    "Longitudinal periodic MPI requires cyclic ghost exchange."
                )
            # Only the decomposed axis changes; preserve transverse conditions.
            self.bc_low = list(self.bc_low)
            self.bc_high = list(self.bc_high)
            if self.rank > 0:
                self.bc_low[2] = "mpi"
        ...
        # Periodic: connect each high face to the corresponding low face.
        for axis, direction in enumerate(["x", "y", "z"]):
            if (self.bc_low[axis].lower() == "periodic") != (
                self.bc_high[axis].lower() == "periodic"
            ):
                raise ValueError(f"Periodic {direction} boundaries must be paired.")

        if any(True for x in self.bc_low if x.lower() == "periodic"):
            # Flat field indices: i + j*Nx + k*Nx*Ny.
            indices = np.arange(self.N).reshape((self.Nx, self.Ny, self.Nz), order="F")

            if (
                self.bc_low[0].lower() == "periodic"
                and self.bc_high[0].lower() == "periodic"
            ):
                # Replace high-x rows: remove connections into the next y line.
                high = indices[-1, :, :].ravel()
                low = indices[0, :, :].ravel()
                self.Px = self.Px.tolil()
                self.Px[high, :] = 0
                self.Px[high, high] = -1
                self.Px[high, low] = 1 if self.Nx > 1 else 0
                self.Px = self.Px.tocsc()

                self.tL[-1, :, :, "x"] = 0.5 * (
                    self.L[-1, :, :, "x"] + self.L[0, :, :, "x"]
                )
            ...
            if (
                ...
                # Replace high-y rows: remove connections into the next z plane.
                high = indices[:, -1, :].ravel()
                low = indices[:, 0, :].ravel()
                self.Py = self.Py.tolil()
                self.Py[high, :] = 0
                self.Py[high, high] = -1
                self.Py[high, low] = 1 if self.Ny > 1 else 0
                self.Py = self.Py.tocsc()

                self.tL[:, -1, :, "y"] = 0.5 * (
                    self.L[:, -1, :, "y"] + self.L[:, 0, :, "y"]
                )
            ...
            if (
                ...
                # Replace high-z rows with last-to-first plane differences.
                high = indices[:, :, -1].ravel()
                low = indices[:, :, 0].ravel()
                self.Pz = self.Pz.tolil()
                self.Pz[high, :] = 0
                self.Pz[high, high] = -1
                self.Pz[high, low] = 1 if self.Nz > 1 else 0
                self.Pz = self.Pz.tocsc()

                self.tL[:, :, -1, "z"] = 0.5 * (
                    self.L[:, :, -1, "z"] + self.L[:, :, 0, "z"]
                )

            # Recompute dual areas after all periodic lengths, including corners.
            ...
            # Rebuild before PEC/PMC masks; CPML also uses the updated P matrices.
            N = self.N
            self.C = vstack(
                [
        ...
        if any(
            ...
            if x.lower() in ("electric", "pec", "pml", "cpml")
        ):
    ...
    def _initialize_PML(self):
        ...
        if self.bc_low[0].lower() == "pml":
            ...
            for d in ["x", "y", "z"]:
                ...
                ieps_0_pml = (
                    1 / eps_0
                )  # self.ieps[self.n_pml + 1, self.Ny // 2, self.Nz // 2, d]
                sigma_0_pml = (
                    0.0  # self.sigma[self.n_pml + 1, self.Ny // 2, self.Nz // 2, d]
                )
                sigma_mult_pml = (
        ...
        if self.bc_low[1].lower() == "pml":
            ...
            for d in ["x", "y", "z"]:
                ...
                ieps_0_pml = (
                    1 / eps_0
                )  # self.ieps[self.Nx // 2, self.n_pml + 1, self.Nz // 2, d]
                sigma_0_pml = (
                    0.0  # self.sigma[self.Nx // 2, self.n_pml + 1, self.Nz // 2, d]
                )
                sigma_mult_pml = (
                    1 if sigma_0_pml < 1 else sigma_0_pml
                )  # avoid null sigma in PML for relaxation time computation
        ...
        if self.bc_low[2].lower() == "pml":
            ...
            for d in ["x", "y", "z"]:
                ...
                ieps_0_pml = (
                    1 / eps_0
                )  # self.ieps[self.Nx // 2, self.Ny // 2, self.n_pml + 1, d]
                sigma_0_pml = (
                    0.0  # self.sigma[self.Nx // 2, self.Ny // 2, self.n_pml + 1, d]
                )
                sigma_mult_pml = (
                    1 if sigma_0_pml < 1 else sigma_0_pml
                )  # avoid null sigma in PML for relaxation time computation
    ...
    def _initialize_CPML(self):
        ...
        if self.bc_low[0].lower() == "cpml":
            ...
            sigma_max = (
                -self.sigma_factor * (self.pml_exp + 1) * np.log(R0) / (2 * L * eta_0)
            )
            for i in range(self.n_pml):
                ...
                tdist = interface - (
                    self.grid.x[i] + self.dx[i] / 2
                )  # distance into PML for half-grid points
        ...
        if self.bc_high[0].lower() == "cpml":
            ...
            for i in range(-self.n_pml, 0):
                ...
                tdist = (
                    self.grid.x[i] + self.dx[i] / 2
                ) - interface  # distance into PML for half-grid points
                tdist = max(0.0, min(tdist, L))
                sx[i] = (dist / L) ** self.pml_exp
                tsx[i] = (tdist / L) ** self.pml_exp
                ax[i] = dist / L
        ...
        if self.bc_high[2].lower() == "cpml":
            ...
            for i in range(-self.n_pml, 0):
                ...
                tdist = (
                    self.grid.z[i] + self.dz[i] / 2 - interface
                )  # distance into PML for half-grid points
                tdist = max(0.0, min(tdist, L))
                sz[i] = (dist / L) ** self.pml_exp
                tsz[i] = (tdist / L) ** self.pml_exp
    ...
    def _initialize_CPML_matrices(self):
        N = self.N
        self.tLx = diags(self.tL.field_x, shape=(N, N), dtype=self.dtype)
        self.tLy = diags(self.tL.field_y, shape=(N, N), dtype=self.dtype)
        self.tLz = diags(self.tL.field_z, shape=(N, N), dtype=self.dtype)
        self.iAx = diags(self.iA.field_x, shape=(N, N), dtype=self.dtype)
        self.iAy = diags(self.iA.field_y, shape=(N, N), dtype=self.dtype)
        self.iAz = diags(self.iA.field_z, shape=(N, N), dtype=self.dtype)
        self.Lx = diags(self.L.field_x, shape=(N, N), dtype=self.dtype)
        self.Ly = diags(self.L.field_y, shape=(N, N), dtype=self.dtype)
        self.Lz = diags(self.L.field_z, shape=(N, N), dtype=self.dtype)
        self.itAx = diags(self.itA.field_x, shape=(N, N), dtype=self.dtype)
        self.itAy = diags(self.itA.field_y, shape=(N, N), dtype=self.dtype)
        self.itAz = diags(self.itA.field_z, shape=(N, N), dtype=self.dtype)
        self.ikapx = diags(1.0 / self.kappa.field_x, shape=(N, N), dtype=self.dtype)
        self.ikapy = diags(1.0 / self.kappa.field_y, shape=(N, N), dtype=self.dtype)
        self.ikapz = diags(1.0 / self.kappa.field_z, shape=(N, N), dtype=self.dtype)
        self.itkapx = diags(1.0 / self.tkappa.field_x, shape=(N, N), dtype=self.dtype)
        self.itkapy = diags(1.0 / self.tkappa.field_y, shape=(N, N), dtype=self.dtype)
        self.itkapz = diags(1.0 / self.tkappa.field_z, shape=(N, N), dtype=self.dtype)
        ...
        self.dtxy = (
            self.itAx * self.ikapy * self.Dbc_x * -self.Py.transpose() * self.tLz
        )
        self.dtxz = (
            self.itAx * self.ikapz * self.Dbc_x * -self.Pz.transpose() * self.tLy
        )
        self.dtyz = (
            self.itAy * self.ikapz * self.Dbc_y * -self.Pz.transpose() * self.tLx
        )
        self.dtyx = (
            self.itAy * self.ikapx * self.Dbc_y * -self.Px.transpose() * self.tLz
        )
        self.dtzx = (
            self.itAz * self.ikapx * self.Dbc_z * -self.Px.transpose() * self.tLy
        )
        self.dtzy = (
        ...
        self.pml_b_H = Field(self.Nx, self.Ny, self.Nz, dtype=self.dtype)
        self.pml_c_H = Field(self.Nx, self.Ny, self.Nz, dtype=self.dtype)
        self.pml_b_E = Field(self.Nx, self.Ny, self.Nz, dtype=self.dtype)
        self.pml_c_E = Field(self.Nx, self.Ny, self.Nz, dtype=self.dtype)

        # Convolution Parameter computation, only valid if sigma is zero in physical domain
        self.pml_b_E.fromarray(
            np.exp(
                -(
                    self.sigmaPml.toarray() / (self.kappa.toarray())
                    + self.alpha.toarray()
                )
                * self.dt
                / eps_0
        ...
        ratio = np.divide(
            self.sigmaPml.toarray(),
            denom,
            out=np.zeros_like(self.sigmaPml.toarray()),
            where=denom != 0,
        )
        self.pml_c_E.fromarray(ratio * (self.pml_b_E.toarray() - 1.0))

        self.pml_b_H.fromarray(
            np.exp(
                -(
                    self.tsigmaPml.toarray() / (self.tkappa.toarray())
                    + self.talpha.toarray()
                )
                * self.dt
                / eps_0
        ...
        ratio = np.divide(
            self.tsigmaPml.toarray(),
            denom,
            out=np.zeros_like(self.tsigmaPml.toarray()),
            where=denom != 0,
        )
        ...
        if self.bc_low[0].lower() == "cpml":
            self.psiHa_z_low = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.psiHb_y_low = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.psiEa_z_low = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.psiEb_y_low = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.pml_b_E_x_low = (self.pml_b_E[: self.n_pml, :, :, "x"].copy()).flatten(
                order="F"
            )
            self.pml_b_H_x_low = (self.pml_b_H[: self.n_pml, :, :, "x"].copy()).flatten(
                order="F"
            )
            self.pml_c_E_x_low = (self.pml_c_E[: self.n_pml, :, :, "x"].copy()).flatten(
                order="F"
            )
            self.pml_c_H_x_low = (self.pml_c_H[: self.n_pml, :, :, "x"].copy()).flatten(
                order="F"
            )
            self.idx_x_low = _get_f_order_indices(
                np.arange(self.n_pml),
                np.arange(self.Ny),
                np.arange(self.Nz),
                self.Nx,
                self.Ny,
        ...
        if self.bc_low[1].lower() == "cpml":
            self.psiHa_x_low = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.psiHb_z_low = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.psiEa_x_low = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.psiEb_z_low = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.pml_c_E_y_low = (self.pml_c_E[:, : self.n_pml, :, "y"].copy()).flatten(
                order="F"
            )
            self.pml_c_H_y_low = (self.pml_c_H[:, : self.n_pml, :, "y"].copy()).flatten(
                order="F"
            )
            self.pml_b_E_y_low = (self.pml_b_E[:, : self.n_pml, :, "y"].copy()).flatten(
                order="F"
            )
            self.pml_b_H_y_low = (self.pml_b_H[:, : self.n_pml, :, "y"].copy()).flatten(
                order="F"
            )
            self.idx_y_low = _get_f_order_indices(
                np.arange(self.Nx),
                np.arange(self.n_pml),
                np.arange(self.Nz),
                self.Nx,
        ...
        if self.bc_low[2].lower() == "cpml":
            self.psiHa_y_low = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.psiHb_x_low = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.psiEa_y_low = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.psiEb_x_low = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.pml_c_E_z_low = (self.pml_c_E[:, :, : self.n_pml, "z"].copy()).flatten(
                order="F"
            )
            self.pml_c_H_z_low = (self.pml_c_H[:, :, : self.n_pml, "z"].copy()).flatten(
                order="F"
            )
            self.pml_b_E_z_low = (self.pml_b_E[:, :, : self.n_pml, "z"].copy()).flatten(
                order="F"
            )
            self.pml_b_H_z_low = (self.pml_b_H[:, :, : self.n_pml, "z"].copy()).flatten(
                order="F"
            )
            self.idx_z_low = _get_f_order_indices(
                np.arange(self.Nx),
                np.arange(self.Ny),
                np.arange(self.n_pml),
                self.Nx,
        ...
        if self.bc_high[0].lower() == "cpml":
            self.psiHa_z_high = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.psiHb_y_high = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.psiEa_z_high = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.psiEb_y_high = np.zeros(
                (self.n_pml * self.Ny * self.Nz), dtype=self.dtype
            )
            self.pml_c_E_x_high = (
                self.pml_c_E[-self.n_pml :, :, :, "x"].copy()
            ).flatten(order="F")
            self.pml_c_H_x_high = (
                self.pml_c_H[-self.n_pml :, :, :, "x"].copy()
            ).flatten(order="F")
            self.pml_b_E_x_high = (
                self.pml_b_E[-self.n_pml :, :, :, "x"].copy()
            ).flatten(order="F")
            self.pml_b_H_x_high = (
                self.pml_b_H[-self.n_pml :, :, :, "x"].copy()
            ).flatten(order="F")
            self.idx_x_high = _get_f_order_indices(
                np.arange(self.Nx - self.n_pml, self.Nx),
                np.arange(self.Ny),
                np.arange(self.Nz),
                self.Nx,
                self.Ny,
        ...
        if self.bc_high[1].lower() == "cpml":
            self.psiHa_x_high = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.psiHb_z_high = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.psiEa_x_high = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.psiEb_z_high = np.zeros(
                (self.Nx * self.n_pml * self.Nz), dtype=self.dtype
            )
            self.pml_c_E_y_high = (
                self.pml_c_E[:, -self.n_pml :, :, "y"].copy()
            ).flatten(order="F")
            self.pml_c_H_y_high = (
                self.pml_c_H[:, -self.n_pml :, :, "y"].copy()
            ).flatten(order="F")
            self.pml_b_E_y_high = (
                self.pml_b_E[:, -self.n_pml :, :, "y"].copy()
            ).flatten(order="F")
            self.pml_b_H_y_high = (
                self.pml_b_H[:, -self.n_pml :, :, "y"].copy()
            ).flatten(order="F")
            self.idx_y_high = _get_f_order_indices(
                np.arange(self.Nx),
                np.arange(self.Ny - self.n_pml, self.Ny),
                np.arange(self.Nz),
                self.Nx,
                self.Ny,
                self.Nz,
        ...
        if self.bc_high[2].lower() == "cpml":
            self.psiHa_y_high = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.psiHb_x_high = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.psiEa_y_high = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.psiEb_x_high = np.zeros(
                (self.Nx * self.Ny * self.n_pml), dtype=self.dtype
            )
            self.pml_c_E_z_high = (
                self.pml_c_E[:, :, -self.n_pml :, "z"].copy()
            ).flatten(order="F")
            self.pml_c_H_z_high = (
                self.pml_c_H[:, :, -self.n_pml :, "z"].copy()
            ).flatten(order="F")
            self.pml_b_E_z_high = (
                self.pml_b_E[:, :, -self.n_pml :, "z"].copy()
            ).flatten(order="F")
            self.pml_b_H_z_high = (
                self.pml_b_H[:, :, -self.n_pml :, "z"].copy()
            ).flatten(order="F")
            self.idx_z_high = _get_f_order_indices(
                np.arange(self.Nx),
                np.arange(self.Ny),
                np.arange(self.Nz - self.n_pml, self.Nz),
                self.Nx,
                self.Ny,
                self.Nz,
            )
